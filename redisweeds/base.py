...
class WeedFS(object):
    ...
    def upload_file(self, path=None, stream=None, name=None, **kwargs):
        ...
        assign_rst = self.conn.get_data(url)
        if assign_rst is None:
            return None

        data = json.loads(assign_rst)
    ...
    def check_volume_status(self, volumeId):
        url = ("http://{master_addr}:{master_port}/dir/lookup").format(
            master_addr=self.master_addr,
            master_port=self.master_port
        )
    ...
    def get_all_volumes(self):
        ...
        vols_info = json.loads(res)
        try:
            return vols_info["Topology"]["layouts"][0]["writables"]
